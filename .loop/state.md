...
- **2026-09-24 Google basemap DONE (Phase B, commit 300e4d8):** Google roadmap + satellite as MAP_STYLES raster entries via Map Tiles API session tokens (`googleTiles.ts`), default 'google' when key configured, voyager fallback on error. Key in `frontend/.env.local` (user's own, project 141791924322 — enabled wrong project first, fixed). Tiles verified: real BKK roadmap TH labels. Zero changes to FS/IMT/marker/polygon logic.
- **2026-09-24 Google search DONE (Phase C, commit a1747d7):** official `<gmp-place-autocomplete>` (TH/TH biased, placeholder ค้นหาสถานที่...) replaces custom input per user ("เหมือน googlemap เลย"), Nominatim fallback on load failure. Polygon drawing confirmed working (draw_polygon → PolygonCreator, basemap-independent). Needs user live-test on ngrok (key referrer-locked).
